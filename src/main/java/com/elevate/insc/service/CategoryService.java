...
public class CategoryService {
    ...
    public ApiResponse<?> updateCategory(String tenantID, UpdateCategoryReqDTO updateCategoryReqDTO) {
        ...
        if (!category.getName().equals(updateCategoryReqDTO.getName()) &&
            categoryRepository.existsByTenantIdAndName(tenantID, updateCategoryReqDTO.getName())) {
            return new ApiResponse<>("Category name already exists in this tenant", 409, null);
        }

        category.setName(updateCategoryReqDTO.getName().trim());
